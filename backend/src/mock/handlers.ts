/**
 * In-memory mock implementations that mirror every real API route.
 * All filtering, sorting and pagination happens here so routes stay thin.
 */
import {
  KLANTEN, PROJECTEN, WERKBONNEN, FACTUREN, INKOOPFACTUREN,
  KOSTENSOORTEN, GROOTBOEK_RUBRIEKEN, GROOTBOEK_MUTATIES,
  type MockFactuur, type MockProject, type MockWerkbon,
  type MockInkoopFactuur, type MockGrootboekMutatie,
} from "./seed.js";
...
function matchSearch(value: unknown, search: string): boolean {
...
export function mockDashboardKpis() {
  ...
  const omzetMonth = FACTUREN
    .filter(f => f.DATUM.startsWith(ym))
    .reduce((s, f) => s + f.BEDRAG_EXCL, 0);

  const omzetYear = FACTUREN
    .filter(f => f.DATUM.startsWith(year))
    .reduce((s, f) => s + f.BEDRAG_EXCL, 0);

  const openProjecten = PROJECTEN.filter(p => p.STATUS === "ACTIEF").length;
  const openWerkbonnen = WERKBONNEN.filter(w => !["AFGEROND", "GEFACTUREERD"].includes(w.STATUS)).length;
  const openDebiteuren = FACTUREN.filter(f => f.OPENSTAAND > 0).reduce((s, f) => s + f.OPENSTAAND, 0);

  return {
    omzetDezeMonth: { OMZET: omzetMonth },
    omzetDitJaar: { OMZET: omzetYear },
    openProjecten: { CNT: openProjecten },
    openWerkbonnen: { CNT: openWerkbonnen },
    openDebiteuren: { BEDRAG: openDebiteuren },
  };
}
...
interface ProjectListParams {
  ...
  search?: string; status?: string; klantId?: number;
...
export function mockProjectenList(p: ProjectListParams) {
  ...
  if (p.search) items = items.filter(r =>
    matchSearch(r.NAAM, p.search!) || matchSearch(r.PROJECTNUMMER, p.search!));
  ...
  const col = (p.sortBy ?? "PROJECTNUMMER") as keyof MockProject;
  items = sortBy(items, col, p.sortDir ?? "DESC");
...
export function mockProjectDetail(id: number) {
  ...
  const werkbonnen = WERKBONNEN
    .filter(w => w.PROJECT_ID === id)
    .map(w => ({ BONNUMMER: w.BONNUMMER, DATUM: w.DATUM, STATUS: w.STATUS, OMSCHRIJVING: w.OMSCHRIJVING }));
  const facturen = FACTUREN
    .filter(f => f.PROJECT_ID === id)
    .map(f => ({ FACTUURNUMMER: f.FACTUURNUMMER, DATUM: f.DATUM, BEDRAG_EXCL: f.BEDRAG_EXCL, BTW: f.BTW, TOTAALBEDRAG: f.TOTAALBEDRAG, STATUS: f.STATUS }));
  return { ...project, werkbonnen, facturen };
}

// ─── Facturen ─────────────────────────────────────────────────────────────────

interface FactuurListParams {
  page?: number; pageSize?: number;
  search?: string; status?: string; klantId?: number; projectId?: number;
  dateFrom?: string; dateTo?: string;
  sortBy?: string; sortDir?: "ASC" | "DESC";
}
...
export function mockFacturenList(p: FactuurListParams) {
  ...
  const col = (p.sortBy ?? "DATUM") as keyof MockFactuur;
  items = sortBy(items, col, p.sortDir ?? "DESC");
  return paginate(items, p.page ?? 1, p.pageSize ?? 50);
}
...
export function mockFactuurDetail(id: number) {
  const f = FACTUREN.find(f => f.ID === id);
  if (!f) return null;
  const klant = KLANTEN.find(k => k.ID === f.KLANT_ID)!;
  // Simulate AT_DOCUMENT regels
  const regels = [
...
export function mockWerkbonnenList(p: WerkbonListParams) {
  ...
  if (p.projectId) items = items.filter(r => r.PROJECT_ID === p.projectId);
  if (p.dateFrom || p.dateTo) items = items.filter(r => inDateRange(r.DATUM, p.dateFrom, p.dateTo));
  const col = (p.sortBy ?? "DATUM") as keyof MockWerkbon;
  items = sortBy(items, col, p.sortDir ?? "DESC");
  return paginate(items, p.page ?? 1, p.pageSize ?? 50);
}

export function mockWerkbonDetail(id: number) {
  const w = WERKBONNEN.find(w => w.ID === id);
  if (!w) return null;
  // Derive realistic regels from totale kosten
  const uurloon = 95;
  const uren = Math.round((w.KOSTEN * 0.65) / uurloon * 2) / 2; // 65% arbeid
...
export function mockWerkbonnenExportData(p: WerkbonListParams) {
  ...
  return items.map(w => ({
    ...
    OMSCHRIJVING:    w.OMSCHRIJVING,
    TYPE:            w.TYPE,
    MOEDERRELATIE:   w.KLANT,
    OBJECTLOCATIE:   w.OBJECTLOCATIE,
    ADRES:           w.ADRES,
    FASE:            w.FASE,
    UITVOERINGSDATUM: w.UITVOERINGSDATUM ?? "",
    COORDINATOR:     w.COORDINATOR,
    KOSTEN:          w.KOSTEN,
    INDIRECT:        w.INDIRECT,
    ALGEMEEN:        w.ALGEMEEN,
    TOTALE_KOSTEN:   w.TOTALE_KOSTEN,
    OPBRENGSTEN:     w.OPBRENGSTEN,
    B_MARGE:         w.B_MARGE,
    MARGE_PCT:       w.MARGE_PCT,
    FACTUURNUMMER:   w.FACTUURNUMMER ?? "",
    FACTUURDATUM:    w.FACTUURDATUM ?? "",
...
export function mockKlantenList(p: KlantListParams) {
  ...
  for (const f of FACTUREN.filter(f => f.DATUM.startsWith(year))) {
    omzetMap.set(f.KLANT_ID, (omzetMap.get(f.KLANT_ID) ?? 0) + f.BEDRAG_EXCL);
  }
  const openMap = new Map<number, number>();
  for (const f of FACTUREN.filter(f => f.OPENSTAAND > 0)) {
    openMap.set(f.KLANT_ID, (openMap.get(f.KLANT_ID) ?? 0) + f.OPENSTAAND);
  }

  let items = KLANTEN.map(k => ({
    ...k,
    OMZET: Math.round((omzetMap.get(k.ID) ?? 0) * 100) / 100,
    OPENSTAAND: Math.round((openMap.get(k.ID) ?? 0) * 100) / 100,
  }));

  if (p.search) items = items.filter(r => matchSearch(r.NAAM, p.search!));

  const dir = p.sortDir ?? "ASC";
  const col = p.sortBy ?? "NAAM";
  items = items.sort((a, b) => {
    const av = col === "OMZET" ? a.OMZET : col === "OPENSTAAND" ? a.OPENSTAAND : a.NAAM;
    const bv = col === "OMZET" ? b.OMZET : col === "OPENSTAAND" ? b.OPENSTAAND : b.NAAM;
    const cmp = av < bv ? -1 : av > bv ? 1 : 0;
    return dir === "DESC" ? -cmp : cmp;
  });
...
export function mockKlantDetail(id: number) {
  ...
  if (!k) return null;
  const projecten = PROJECTEN.filter(p => p.KLANT_ID === id)
    .map(p => ({ PROJECTNUMMER: p.PROJECTNUMMER, NAAM: p.NAAM, STATUS: p.STATUS, STARTDATUM: p.STARTDATUM, EINDDATUM: p.EINDDATUM }));
  const facturen = FACTUREN.filter(f => f.KLANT_ID === id)
    .sort((a, b) => b.DATUM.localeCompare(a.DATUM))
    .slice(0, 20)
    .map(f => ({ FACTUURNUMMER: f.FACTUURNUMMER, DATUM: f.DATUM, TOTAALBEDRAG: f.TOTAALBEDRAG, OPENSTAAND: f.OPENSTAAND, STATUS: f.STATUS }));
  return { ...k, projecten, facturen };
}

// ─── Inkoop ───────────────────────────────────────────────────────────────────

interface InkoopListParams {
  page?: number; pageSize?: number;
  search?: string; projectId?: number; leverancierId?: number;
  dateFrom?: string; dateTo?: string;
  sortBy?: string; sortDir?: "ASC" | "DESC";
}
...
export function mockInkoopList(p: InkoopListParams) {
  ...
  if (p.dateFrom || p.dateTo) items = items.filter(r => inDateRange(r.DATUM, p.dateFrom, p.dateTo));
  const col = (p.sortBy ?? "DATUM") as keyof MockInkoopFactuur;
  items = sortBy(items, col, p.sortDir ?? "DESC");
  return paginate(items, p.page ?? 1, p.pageSize ?? 50);
}
...
export function mockInkoopPerKostensoort() {
  ...
  return Array.from(map.entries())
    .sort(([, a], [, b]) => b - a)
    .map(([kostensoort, bedrag]) => ({ KOSTENSOORT: kostensoort, BEDRAG: Math.round(bedrag * 100) / 100 }));
}

// ─── Grootboek ────────────────────────────────────────────────────────────────

interface MutatieListParams {
  page?: number; pageSize?: number;
  rubriekId?: number; dateFrom?: string; dateTo?: string;
  sortDir?: "ASC" | "DESC";
}

export function mockGrootboekRubrieken() {
  return GROOTBOEK_RUBRIEKEN;
}
...
export function mockGrootboekResultaat() {
  ...
  return GROOTBOEK_RUBRIEKEN.map(r => {
    const rMutaties = mutaties.filter(m => m.RUBRIEK_ID === r.ID);
    const debet = rMutaties.reduce((s, m) => s + m.DEBET, 0);
    const credit = rMutaties.reduce((s, m) => s + m.CREDIT, 0);
    return {
      SOORT: r.SOORT,
      REKENINGNUMMER: r.REKENINGNUMMER,
      OMSCHRIJVING: r.OMSCHRIJVING,
      DEBET_TOTAAL: Math.round(debet * 100) / 100,
      CREDIT_TOTAAL: Math.round(credit * 100) / 100,
      SALDO: Math.round((credit - debet) * 100) / 100,
    };
  });
}
...
export function mockRapportOmzetProject(dateFrom?: string, dateTo?: string) {
  ...
  for (const f of FACTUREN) {
    ...
    const proj = PROJECTEN.find(p => p.ID === f.PROJECT_ID);
    if (!proj) continue;
    const entry = map.get(proj.PROJECTNUMMER)!;
    entry.OMZET = Math.round((entry.OMZET + f.BEDRAG_EXCL) * 100) / 100;
  }
  return Array.from(map.values())
    .filter(e => e.OMZET > 0)
    .sort((a, b) => b.OMZET - a.OMZET)
    .map((e, i) => ({ PROJECTNUMMER: PROJECTEN[i]?.PROJECTNUMMER ?? "—", ...e }));
}

export function mockRapportOpenDebiteuren() {
  return FACTUREN
    .filter(f => f.OPENSTAAND > 0)
    .sort((a, b) => b.DAGEN_OVERDUE - a.DAGEN_OVERDUE)
    .map(f => ({
      KLANT: f.KLANT,
      FACTUURNUMMER: f.FACTUURNUMMER,
      DATUM: f.DATUM,
      VERVALDATUM: f.VERVALDATUM,
      TOTAALBEDRAG: f.TOTAALBEDRAG,
      OPENSTAAND: f.OPENSTAAND,
      DAGEN_OVERDUE: f.DAGEN_OVERDUE,
    }));
}
...
export function mockRapportMargeProjectleider(dateFrom?: string, dateTo?: string) {
  ...
  for (const p of PROJECTEN) {
    ...
    const entry = map.get(p.PROJECTLEIDER) ?? { PROJECTEN: 0, OMZET: 0, KOSTEN: 0 };
    entry.PROJECTEN++;
    entry.OMZET += FACTUREN.filter(f => f.PROJECT_ID === p.ID).reduce((s, f) => s + f.BEDRAG_EXCL, 0);
    entry.KOSTEN += INKOOPFACTUREN.filter(i => i.PROJECT_ID === p.ID).reduce((s, i) => s + i.BEDRAG_EXCL, 0);
    map.set(p.PROJECTLEIDER, entry);
  }
  return Array.from(map.entries())
    .sort(([, a], [, b]) => (b.OMZET - b.KOSTEN) - (a.OMZET - a.KOSTEN))
    .map(([pl, e]) => ({
      PROJECTLEIDER: pl,
      PROJECTEN: e.PROJECTEN,
      OMZET: Math.round(e.OMZET * 100) / 100,
      KOSTEN: Math.round(e.KOSTEN * 100) / 100,
      MARGE: Math.round((e.OMZET - e.KOSTEN) * 100) / 100,
    }));
}
...
export function mockRapportInkoopKostensoort(dateFrom?: string, dateTo?: string) {
  ...
  for (const f of INKOOPFACTUREN) {
    ...
    e.FACTUREN++;
    e.BEDRAG_EXCL += f.BEDRAG_EXCL;
    e.BTW += f.BTW;
    e.TOTAAL += f.TOTAALBEDRAG;
    map.set(ks, e);
  }
  return Array.from(map.entries())
    .sort(([, a], [, b]) => b.BEDRAG_EXCL - a.BEDRAG_EXCL)
    .map(([ks, e]) => ({
      KOSTENSOORT: ks,
      FACTUREN: e.FACTUREN,
      BEDRAG_EXCL: Math.round(e.BEDRAG_EXCL * 100) / 100,
      BTW: Math.round(e.BTW * 100) / 100,
      TOTAAL: Math.round(e.TOTAAL * 100) / 100,
    }));
}

// ─── Medewerkers & Uren ────────────────────────────────────────────────────────

import { MEDEWERKERS, UREN } from "./seed.js";

export function mockMedewerkers() {
  return MEDEWERKERS.filter(m => m.ACTIEF).map(m => ({
    ID: m.ID, GC_CODE: m.GC_CODE, NAAM: m.NAAM, FUNCTIE: m.FUNCTIE, UURLOON: m.UURLOON,
    UREN_DEZE_WEEK: UREN.filter(u => {
      const d = new Date(u.DATUM);
      const now = new Date("2026-05-13");
      const dow = now.getDay() || 7;
      const mon = new Date(now); mon.setDate(now.getDate() - dow + 1);
      const sun = new Date(mon); sun.setDate(mon.getDate() + 6);
      return u.MEDEWERKER_ID === m.ID && d >= mon && d <= sun;
    }).reduce((s, u) => s + u.AANTAL, 0),
  }));
}

export function mockUrenPerWeek() {
  // Uren per dag afgelopen 2 weken
  const map = new Map<string, number>();
  const cutoff = "2026-04-29";
  for (const u of UREN.filter(u => u.DATUM >= cutoff)) {
...
export function mockUrenPerProject() {
  ...
  const map = new Map<string, { NAAM: string; UREN: number }>();
  for (const u of UREN.filter(u => u.DATUM.startsWith(month))) {
    const key = u.WERK_CODE;
    const e = map.get(key) ?? { NAAM: u.WERK_NAAM.length > 30 ? u.WERK_NAAM.slice(0, 28) + "…" : u.WERK_NAAM, UREN: 0 };
    e.UREN = Math.round((e.UREN + u.AANTAL) * 10) / 10;
    map.set(key, e);
  }
  return Array.from(map.entries())
...
export function mockRecenteUren() {
  return [...UREN]
    ...
    .slice(0, 15)
    .map(u => ({
      DATUM: u.DATUM, MEDEWERKER: u.MEDEWERKER, GC_CODE: u.GC_CODE,
      WERK_CODE: u.WERK_CODE, WERK_NAAM: u.WERK_NAAM,
      TAAK: u.TAAK, AANTAL: u.AANTAL, OMSCHRIJVING: u.OMSCHRIJVING,
    }));
}

export function mockDashboardUrenStats() {
  const month = "2026-05";
  const week  = ["2026-05-13","2026-05-14","2026-05-15","2026-05-16","2026-05-17"];
  return {
    UREN_DEZE_WEEK:  UREN.filter(u => week.includes(u.DATUM)).reduce((s, u) => s + u.AANTAL, 0),
