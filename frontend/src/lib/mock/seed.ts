...
export interface MockKlant {
  ID: number; KLANTNUMMER: string; NAAM: string; ADRES: string;
  POSTCODE: string; PLAATS: string; EMAIL: string; TELEFOON: string;
}
export interface MockProject {
  ID: number; PROJECTNUMMER: string; NAAM: string; KLANT_ID: number; KLANT: string;
  STATUS: "ACTIEF" | "AFGEROND" | "GEANNULEERD" | "ON_HOLD";
  STARTDATUM: string; EINDDATUM: string | null; PROJECTLEIDER: string; OMSCHRIJVING: string;
}
export interface MockWerkbon {
  ID: number; BONNUMMER: string; DATUM: string;
  STATUS: "NIEUW" | "IN_UITVOERING" | "AFGEROND" | "GEFACTUREERD";
  OMSCHRIJVING: string; TYPE: string; KLANT_ID: number; KLANT: string;
  OBJECTLOCATIE: string; ADRES: string; FASE: string; UITVOERINGSDATUM: string | null;
  COORDINATOR: string; PROJECT_ID: number | null; PROJECTNUMMER: string | null; PROJECT_NAAM: string | null;
  KOSTEN: number; INDIRECT: number; ALGEMEEN: number; TOTALE_KOSTEN: number;
  OPBRENGSTEN: number; B_MARGE: number; MARGE_PCT: number;
  FACTUURNUMMER: string | null; FACTUURDATUM: string | null; BETAALD: string | null;
}
export interface MockFactuur {
  ID: number; FACTUURNUMMER: string; DATUM: string; VERVALDATUM: string;
  KLANT_ID: number; KLANT: string; PROJECT_ID: number | null;
  BEDRAG_EXCL: number; BTW: number; TOTAALBEDRAG: number; OPENSTAAND: number;
  STATUS: "OPEN" | "BETAALD" | "DEELS_BETAALD"; DAGEN_OVERDUE: number;
}
export interface MockInkoopFactuur {
  ID: number; FACTUURNUMMER: string; DATUM: string; LEVERANCIER_ID: number;
  LEVERANCIER: string; PROJECT_ID: number | null; KOSTENSOORT_ID: number;
  BEDRAG_EXCL: number; BTW: number; TOTAALBEDRAG: number; STATUS: "NIEUW" | "GEBOEKT";
}
export interface MockGrootboekRubriek {
  ID: number; REKENINGNUMMER: string; OMSCHRIJVING: string; SOORT: "BALANS" | "RESULTAAT";
}
export interface MockGrootboekMutatie {
  ID: number; DATUM: string; OMSCHRIJVING: string; DEBET: number; CREDIT: number;
  RUBRIEK_ID: number; REKENINGNUMMER: string; RUBRIEK: string;
}
export interface MockMedewerker {
  ID: number; GC_CODE: string; NAAM: string; FUNCTIE: string; UURLOON: number; ACTIEF: boolean;
}
export interface MockUrenRegel {
  ID: number; DATUM: string; MEDEWERKER_ID: number; MEDEWERKER: string; GC_CODE: string;
  WERK_GC_ID: number; WERK_CODE: string; WERK_NAAM: string; TAAK: string; AANTAL: number; OMSCHRIJVING: string;
}
...
function wb(
  id: number, nr: string, datum: string, status: MockWerkbon["STATUS"],
  omschrijving: string, type: string, klantId: number, klant: string,
  objectlocatie: string, adres: string, fase: string, uitvoeringsdatum: string | null,
  coordinator: string, projectId: number | null, projectnummer: string | null, projectNaam: string | null,
  ...
  return {
    ID: id, BONNUMMER: nr, DATUM: datum, STATUS: status, OMSCHRIJVING: omschrijving, TYPE: type,
    ...
    KOSTEN: kosten, INDIRECT: indirect, ALGEMEEN: algemeen, TOTALE_KOSTEN: totaal,
    OPBRENGSTEN: opbrengsten, B_MARGE: marge, MARGE_PCT: margeP,
...
function daysOverdue(vervaldatum: string, openstaand: number): number {
  ...
  return Math.max(0, Math.floor((Date.now() - new Date(vervaldatum).getTime()) / 86400000));
}

const RAW_FACTUREN: { ID: number; DATUM: string; KLANT_ID: number; BEDRAG_EXCL: number; PROJECT_ID: number | null; betaald: boolean; deels?: boolean }[] = [
...
export const FACTUREN: MockFactuur[] = RAW_FACTUREN.map((r, i) => {
  ...
  return {
    ID: r.ID, FACTUURNUMMER: `F-2026-${String(i + 1).padStart(4, "0")}`,
    DATUM: r.DATUM, VERVALDATUM: vervaldatum, KLANT_ID: r.KLANT_ID, KLANT: klant.NAAM,
    PROJECT_ID: r.PROJECT_ID, BEDRAG_EXCL: r.BEDRAG_EXCL, BTW: btw, TOTAALBEDRAG: totaal,
    OPENSTAAND: openstaand, STATUS: r.betaald ? "BETAALD" : r.deels ? "DEELS_BETAALD" : "OPEN",
    DAGEN_OVERDUE: daysOverdue(vervaldatum, openstaand),
  } as MockFactuur;
});

const LEVERANCIERS = [
  { ID: 1, NAAM: "TechniekGrossier NL BV" }, { ID: 2, NAAM: "Warmtepomp Specialist BV" },
  { ID: 3, NAAM: "Elektra Groothandel West" }, { ID: 4, NAAM: "Loodgieter Supplies BV" },
  { ID: 5, NAAM: "Kabel & Draad Direct" }, { ID: 6, NAAM: "Klimaatcentrum Holland" },
];
export const KOSTENSOORTEN = [
  { ID: 1, OMSCHRIJVING: "Materialen" }, { ID: 2, OMSCHRIJVING: "Onderdelen" },
  { ID: 3, OMSCHRIJVING: "Inhuur personeel" }, { ID: 4, OMSCHRIJVING: "Transport" },
  { ID: 5, OMSCHRIJVING: "Gereedschap" }, { ID: 6, OMSCHRIJVING: "Overig" },
];
const RAW_INKOOP: { ID: number; DATUM: string; LEV_ID: number; KS_ID: number; BEDRAG_EXCL: number; PROJECT_ID: number | null }[] = [
...
export const INKOOPFACTUREN: MockInkoopFactuur[] = RAW_INKOOP.map((r, i) => {
  ...
  const btw = Math.round(r.BEDRAG_EXCL * 0.21 * 100) / 100;
  return {
    ID: r.ID, FACTUURNUMMER: `INK-2026-${String(i + 1).padStart(4, "0")}`,
    DATUM: r.DATUM, LEVERANCIER_ID: r.LEV_ID, LEVERANCIER: lev.NAAM,
    PROJECT_ID: r.PROJECT_ID, KOSTENSOORT_ID: r.KS_ID,
    BEDRAG_EXCL: r.BEDRAG_EXCL, BTW: btw, TOTAALBEDRAG: r.BEDRAG_EXCL + btw,
    STATUS: r.ID <= 10 ? "GEBOEKT" : "NIEUW",
  } as MockInkoopFactuur;
});
...
export const GROOTBOEK_MUTATIES: MockGrootboekMutatie[] = [
  ...
  { ID: 5,  DATUM: "2026-05-05", OMSCHRIJVING: "Verkoopfactuur ZA-003 april",           DEBET: 11918.5, CREDIT: 0,       RUBRIEK_ID: 4,  REKENINGNUMMER: "1300", RUBRIEK: "Debiteuren"        },
  ...
  { ID: 11, DATUM: "2026-04-10", OMSCHRIJVING: "Betaling debiteur Gemeente Rotterdam",  DEBET: 0,       CREDIT: 22712.5, RUBRIEK_ID: 4,  REKENINGNUMMER: "1300", RUBRIEK: "Debiteuren"        },
  { ID: 12, DATUM: "2026-04-10", OMSCHRIJVING: "Betaling debiteur Gemeente Rotterdam",  DEBET: 22712.5, CREDIT: 0,       RUBRIEK_ID: 3,  REKENINGNUMMER: "1000", RUBRIEK: "Kas"               },
