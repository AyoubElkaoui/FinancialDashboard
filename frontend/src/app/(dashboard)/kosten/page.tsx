  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle className="text-sm">Verdeling per kostensoort</CardTitle></CardHeader>
          <CardContent>
            {isLoading ? <div className="h-64 animate-pulse bg-muted rounded" /> : (
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  <Pie data={rows} dataKey="BEDRAG" nameKey="KOSTENSOORT" cx="50%" cy="50%" outerRadius={100} label={(p) => `${p.name} ${((p.percent ?? 0) * 100).toFixed(0)}%`} labelLine={false}>
        ...
        <Card>
          <CardHeader><CardTitle className="text-sm">Bedrag per kostensoort</CardTitle></CardHeader>
          <CardContent>
            {isLoading ? <div className="h-64 animate-pulse bg-muted rounded" /> : (
              <ResponsiveContainer width="100%" height={280}>
      ...
      <Card>
        <CardHeader><CardTitle className="text-sm">Detail per kostensoort</CardTitle></CardHeader>
        <CardContent className="p-0">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 border-b">
              <tr>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground">Kostensoort</th>
