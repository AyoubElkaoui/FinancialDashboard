...
export async function GET(
  ...
  const database = (req.nextUrl.searchParams.get("database") ?? "SERVICES") as Database;

  // Zoek in read-model op projectnummer (ID = string projectcode, bv. "070-25-002")
  let rm = null;
  try {
    ...
    // DB fout: val terug op mock
  }

  if (!rm) {
    const rapport = getElmarRapport(Number(id), database as string);
    if (rapport) return Response.json(rapport);
    return Response.json({ error: "Project niet gevonden" }, { status: 404 });
  }

  // App-parameters voor berekeningen (optioneel — geen fout als niet beschikbaar)
  let input = null;
  try {
    input = await db.projectInput.findUnique({
      where: { database_projectCode: { database, projectCode: id } },
    });
  } catch { /* geen app-parameters beschikbaar */ }
  ...
  let journaalOmzet: { rubriekCode: string; datum: unknown; bedrag: unknown; omschrijving: string | null }[] = [];
  try {
  ...
  } catch { /* geen journaaldata beschikbaar */ }
  ...
  return Response.json({
    ...
    // Backwards compat (AT_AANMVFAC leeg — geen betalingsdata)
